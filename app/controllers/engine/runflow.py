...
from app.controllers.engine.tools.conditional.classify_message import (
    CLASSIFY_MESSAGE,
    classify_message,
)
...
async def convert_edges_to_nodes(edges: List[UserEdge]) -> List[Node]:
    ...
    for source_node_id, targets in node_map.items():
        ...
        print("node_id: ", node_id, source_node_id)
        found_node = tools_db.get_by_id(node_id)
...
async def create_graph(nodes: List[Node]) -> CompiledStateGraph:
    ...
    for node in nodes:
        ...
        node_data = tools_db.get_by_id(node_db_id)
        ...
        if is_conditional:
            ...
            data = NodeData(
                node_graph_id=classify_node_id,
                next_nodes=next_nodes_graph_id,
                node_input=node_input,
            )
            graph_builder.add_node(
                classify_node_id, with_node_data(data=data, tool=classify_message)
            )
            classify_count = classify_count + 1

        if node_data.category != "initiate":
            print(f"63: graph_builder.add_node({node_graph_id},{node_type_tool})")
            req.get("next_node_id")
            data = NodeData(
                node_graph_id=node_graph_id,
                next_nodes=req.get("next_node_id"),
                node_input=node_input,
            )
            graph_builder.add_node(
                node_graph_id, with_node_data(data=data, tool=node_type_tool)
            )
    ...
    for node in nodes:
        ...
        next_edge_data = tools_db.get_by_id(next_edge_db_id)

        if is_start:
            if next_edge_data.category == "conditional":
            ...
            print(
                f"89: graph_builder.add_conditional_edges({classification_id}, {next_routing_tool})"
            )
            ...
            if next_edge_data.category == "conditional":
                ...
                # manage classification id
                print(
                    f"96: graph_builder.add_edge({start_edge_graph_id},{classification_id})"
                )
                graph_builder.add_edge(start_edge_graph_id, classification_id)
            elif next_edge_data.category == "close":
                print(f"99: graph_builder.add_edge({start_edge_graph_id},{END})")
                graph_builder.add_edge(start_edge_graph_id, END)
            else:
                print(
                    f"102: graph_builder.add_edge({start_edge_graph_id},{next_edge_graph_id})"
                )
